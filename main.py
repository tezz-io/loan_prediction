...
class App:
	def __init__(self, loan_id, gender, is_married, dependents, edu, is_self_employed, app_income, co_income, loan_amt, loan_term, credit_score, _property, loan_approved):
		self.id = int(loan_id)
		self.gender = gender
		self.marriage = Married(is_married)
		self.dependents = Dependents(dependents)
		...
		self.loan_term = LoanTerm(loan_term)
		self.credit_score = CreditScore(credit_score)
		self.property = Property(_property)
		self.loan_approved = LoanApproved(loan_approved)

	def calculate_score(self, ):
		return (self.married*50 + self.dependents*25 + self.edu*25 + self.self_emp*50)
